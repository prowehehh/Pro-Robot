...
const commands = [
    ...
    new SlashCommandBuilder().setName('ads_edit').setDescription('Edit or Stop an advertisement')
        .addStringOption(o => o.setName('name').setDescription('Ad Name to edit/delete').setRequired(true).setAutocomplete(true)),
...
client.on('messageCreate', async (message) => {
    ...
    if (badWordsPattern.some(p => p.test(content)) || /(https?:\/\/[^\s]+)/g.test(content) || scamKeywords.some(k => content.includes(k)) || message.attachments.some(file => dangerousExts.some(ext => file.name.toLowerCase().endsWith(ext)))) {
        ...
        if (count >= 2) {
            ...
            const warn = await message.channel.send(`🚫 <@${message.author.id}>، غير مسموح بهذا!`);
    ...
    if (interaction.isChatInputCommand()) {
        ...
        if (commandName === 'ads_edit') {
            ...
            const row = new ActionRowBuilder().addComponents(
                new ButtonBuilder().setCustomId(`stop_ad_${name}`).setLabel('Stop & Delete Ad 🗑️').setStyle(ButtonStyle.Danger)
            );
            await interaction.reply({ content: `Manage Ad: **${name}**`, components: [row], ephemeral: true });
        }
        ...
        if (interaction.customId.startsWith('stop_ad_')) {
            ...
            if (ad) {
                ...
                await interaction.reply({ content: `🗑️ Ad **${name}** has been stopped and deleted from system.`, ephemeral: true });
            }
        } else {
