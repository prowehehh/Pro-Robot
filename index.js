    ...
    try {
        const response = await fetch('https://api.mistral.ai/v1/chat/completions', {
            ...
            body: JSON.stringify({
                model: "mistral-tiny",
                messages: [
                    { role: "system", content: `You are an AI for "Pro Security System". Support all languages. Server Info: Created 15/03/2026, Location: Egypt, Owner: <@${CONFIG.OWNER_ID}>. Rules: No insults/swearing, no advertising.` },
                    { role: "user", content: userMessage }
                ]
            })
        });
        const data = await response.json();
        return data.choices?.[0]?.message?.content || "انا لا اعرف اسال صاحب السيرفر <@1134146616857731173>";
    } catch (err) {
        return "انا لا اعرف اسال صاحب السيرفر <@1134146616857731173>";
    }
}
