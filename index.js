    ...
    try {
        const response = await fetch('https://api.mistral.ai/v1/chat/completions', {
            ...
            body: JSON.stringify({
                model: "mistral-small",
                messages: [
                    { role: "system", content: `You are "Pro AI", a highly professional assistant. Support all world languages. 
                    - Personality: Elite, smart, helpful, and fast.
                    - Server Info: Created 15/03/2026, Location: Egypt, Owner: Saif (<@${CONFIG.OWNER_ID}>).
                    - Knowledge: You know all server ranks (@Ultimate, @YouTuber, @Vip, @Helper) and rules.
                    - Tasks: Guide users to the Submit button for ranks. Answer greetings politely.
                    - Fail-safe: If you don't know, say: "انا لا اعرف اسال صاحب السيرفر <@${CONFIG.OWNER_ID}>".` },
...
const commands = [
    ...
    new SlashCommandBuilder().setName('ads_edit').setDescription('تعديل أو حذف إعلان قائم').addStringOption(o => o.setName('name').setDescription('اسم الإعلان').setRequired(true).setAutocomplete(true)).addStringOption(o => o.setName('text').setDescription('النص الجديد').setRequired(false)).addChannelOption(o => o.setName('channel').setDescription('القناة الجديدة').addChannelTypes(ChannelType.GuildText).setRequired(false)).addIntegerOption(o => o.setName('interval').setDescription('الوقت الجديد').setRequired(false)).addIntegerOption(o => o.setName('delete').setDescription('وقت الحذف الجديد').setRequired(false)).addStringOption(o => o.setName('style').setDescription('الشكل الجديد').setRequired(false).addChoices({name:'Box',value:'embed'},{name:'Normal',value:'normal'})),
    new SlashCommandBuilder().setName('translate').setDescription('ترجمة نصوص').addStringOption(o => o.setName('text').setDescription('النص').setRequired(true)).addStringOption(o => o.setName('to').setDescription('كود اللغة').setRequired(true)),
    ...
    try {
        ...
        if (text) {
            ...
            setTimeout(() => {
                ...
                botMsg.delete().catch(() => {});
            }, 300000); // حذف تلقائي بعد 5 دقائق
        }
        if (['rank', 'رتبة', 'رتبه'].some(k => message.content.toLowerCase().includes(k))) {
            ...
            const sentM = await message.channel.send({ embeds: [embed], components: [row] });
            setTimeout(() => sentM.delete().catch(() => {}), 300000);
        }
...
client.on('interactionCreate', async (interaction) => {
    if (interaction.isButton() && interaction.customId === 'open_rank_modal') {
        ...
        const userField = new TextInputBuilder().setCustomId('xbox_user').setLabel("Username").setStyle(TextInputStyle.Short).setPlaceholder("Write Xbox user").setRequired(true);
        const rankField = new TextInputBuilder().setCustomId('rank_type').setLabel("Rank you want").setStyle(TextInputStyle.Short).setPlaceholder("Rank name").setRequired(true);
    ...
    if (interaction.isChatInputCommand()) {
        ...
        try {
            ...
            if (commandName === 'send') {
                const msg = options.getString('message'), style = options.getString('style'), delay = options.getInteger('delay_send'), delAfter = options.getInteger('delete_after'), color = options.getString('color') || '#3498db';
                await interaction.reply({ content: `✅ ستصل الرسالة خلال ${delay} دقيقة.`, ephemeral: true });
                setTimeout(async () => {
                    let sent; if (style === 'embed') { sent = await channel.send({ embeds: [new EmbedBuilder().setDescription(msg).setColor(color)] }).catch(() => {}); } else { sent = await channel.send(msg).catch(() => {}); }
                    if (sent && delAfter > 0) setTimeout(() => sent.delete().catch(() => {}), delAfter * 60000);
                }, delay * 60000);
            }
            if (commandName === 'ads_set') {
                const name = options.getString('name'), data = { name, text: options.getString('text'), channelId: options.getChannel('channel').id, interval: options.getInteger('interval'), deleteAfter: options.getInteger('delete'), style: options.getString('style'), timer: null, lastMsgId: null };
                adsStorage.set(name, data); startAdLoop(name, guild.id); return await interaction.reply({ content: `✅ تم تفعيل إعلان: **${name}**`, ephemeral: true });
            }
            if (commandName === 'ads_edit') {
                const name = options.getString('name'), ad = adsStorage.get(name); if (!ad) return await interaction.reply({ content: "❌ غير موجود.", ephemeral: true });
                if (options.getString('text')) ad.text = options.getString('text'); if (options.getChannel('channel')) ad.channelId = options.getChannel('channel').id; if (options.getInteger('interval')) ad.interval = options.getInteger('interval'); if (options.getInteger('delete') !== null) ad.deleteAfter = options.getInteger('delete'); if (options.getString('style')) ad.style = options.getString('style');
                startAdLoop(name, guild.id); const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`stop_ad_${name}`).setLabel('حذف الإعلان 🗑️').setStyle(ButtonStyle.Danger)); return await interaction.reply({ content: `⚙️ تم تحديث **${name}**.`, components: [row], ephemeral: true });
            }
            if (commandName === 'clear') { await interaction.deferReply({ ephemeral: true }); await channel.bulkDelete(Math.min(options.getInteger('amount'), 100)).catch(() => {}); return await interaction.editReply('تم تنظيف الشات! 🧹'); }
            if (commandName === 'translate') { await interaction.deferReply(); const res = await fetch(`https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=${options.getString('to').toLowerCase()}&dt=t&q=${encodeURI(options.getString('text'))}`), json = await res.json(); return await interaction.editReply({ embeds: [new EmbedBuilder().setTitle('🌐 ترجمة').setDescription(json[0].map(i => i[0]).join('')).setColor('#4285F4')] }); }
            if (commandName === 'vote') { const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('v_yes').setLabel('نعم ✅').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId('v_no').setLabel('لا ❌').setStyle(ButtonStyle.Danger)); return await interaction.reply({ embeds: [new EmbedBuilder().setTitle('تصويت جديد').setDescription(options.getString('question')).setColor('#f1c40f')], components: [row] }); }
        } catch (e) { console.error(e); }
    } else if (interaction.isButton() && interaction.customId.startsWith('stop_ad_')) {
        const name = interaction.customId.replace('stop_ad_', ''), ad = adsStorage.get(name); if (ad) { if (ad.timer) clearInterval(ad.timer); adsStorage.delete(name); await interaction.update({ content: `🗑️ تم حذف **${name}**`, components: [], ephemeral: true }); }
    }
});

client.on('guildMemberAdd', async (member) => {
    const role = member.guild.roles.cache.get(CONFIG.AUTO_ROLE); if (role) await member.roles.add(role).catch(() => {});
...
async function updateLiveInfo(guild) {
    if (!guild) guild = client.guilds.cache.first(); const infoCh = client.channels.cache.get(CONFIG.INFO_CH); if (!infoCh || !guild) return;
    const infoEmbed = new EmbedBuilder().setDescription(`[!]≈≈≈≈≈≈≈≈≈≈≈≈≈|!|≈≈≈≈≈≈≈≈≈≈≈≈≈[!]\n• Owner: <@${CONFIG.OWNER_ID}>\n• Members: ${guild.memberCount}\n[!]≈≈≈≈≈≈≈≈≈≈≈≈≈|!|≈≈≈≈≈≈≈≈≈≈≈≈≈[!]`).setColor('#3498db');
    try {
        const msgs = await infoCh.messages.fetch({ limit: 10 }).catch(() => null); if (msgs) { msgs.filter(m => m.author.id === client.user.id).forEach(async m => await m.delete().catch(() => {})); }
